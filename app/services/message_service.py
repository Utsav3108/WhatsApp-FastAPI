...
async def get_messages_by_persona_session_id(db: AsyncSession, persona_session_id: int, limit: int | None = 10) -> List[MessageResponse]:
    messages = await crud.get_messages_by_persona_session_id(db, persona_session_id, limit=limit)
    return [MessageResponse.model_validate(m) for m in messages]